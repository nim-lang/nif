...
proc matchCallCanRaise(c: var Context; it: var Item): bool =
  ...
  if not kw2: return false
  when declared(handleCallCanRaise):
    handleCallCanRaise(c, it, before1)
  return true
...
proc matchTryStmt(c: var Context; it: var Item): bool =
  ...
  if not kw2: return false
  when declared(handleTryStmt):
    handleTryStmt(c, it, before1)
  return true

proc matchRaiseStmt(c: var Context; it: var Item): bool =
  when declared(handleRaiseStmt):
    var before1 = save(c, it)
  var kw2 = false
  if isTag(c, it, RaiseT):
    var or3 = false
    block or4:
      if matchEmpty(c, it):
        or3 = true
